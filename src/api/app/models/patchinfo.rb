...
class Patchinfo < ActiveXML::Node
  ...
  #def create_patchinfo_from_request(project, req, pkg_name, diff, issues)
  def create_patchinfo_from_request(project, req, pkg_name, diff, issues, issueList)
    project.check_write_access!
    @prj = project
    ...
    issueList.each do |issue|
      e = xml.add_element 'issue'
      issName = issue.attributes['name']
      issTracker = issue.attributes['tracker']
      e.set_attribute 'tracker', issTracker
      e.set_attribute 'id', issName
      cnt += 1
    end
   ...
   # diff = new_diff.join("\n")
   #
   # reqdiff = req.bs_request_actions.first.sourcediff(view:"xml",withissues:true)
   ...
   # end
    ...
    #description = "\nThis update fixes the following issues with #{pkg_name}:\n#{diff}"
    description = "This update fixes the following issues with #{pkg_name}:\n#{diff}"
    xml.add_element('packager').text = req.creator
    xml.add_element('summary').text = pkg_name + ": Fixes " + cnt.to_s + " bugs." # category
  ...
  def fetch_release_targets(pkg)
    ...
    if data.empty?
      raise IncompletePatchinfo.new 'The _patchinfo file is not parsable'
    end
